...
```bash
hermes$ sam build --use-container
```
...
```bash
hermes$ sam logs -n StockCheckerFunction --stack-name "hermes" --tail
```
...
```bash
hermes$ pip install -r tests/requirements.txt --user
# unit test
hermes$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
hermes$ AWS_SAM_STACK_NAME="hermes" python -m pytest tests/integration -v
```
